...
const cardsSchema = Yup.object().shape({
  ...
  groupDescription: Yup.string().max(300, "Group Description Must be less than 300 characters").min(20, "Group Description Must be More Than 20 Characters").required('Group description is required'),
  cards: Yup.array().of(
    Yup.object().shape({
      name: Yup.string().max(20, "Card name Must be less than 20 characters").min(3, "Card name Must be More Than 2 Characters").required('Card name is required'),
      image: Yup.mixed(),
